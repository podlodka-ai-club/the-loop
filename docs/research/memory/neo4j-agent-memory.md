...
- `Location` nodes, temporal relations и provenance хорошо совпадают с задачами geolocation.
  Markdown training experience можно передавать в native extraction как message/observation.
- Registry сопоставляет `memory_ref` с workspace/endpoint/graph. Для inference следует
...
- Какой native graph/context view даёт лучший результат агенту Loci?
